Extensions to the base PMIx Standard are listed here for each OpenPMIx release. These extensions
...
it is expected that the extensions listed below will make their way into a future release of the
PMIx Standard and then be removed from the list of extensions in some future OpenPMIx release.
...
OpenPMIx v4.1.0 - Exceptions to the PMIx v4.1 Standard
---------------------------------
...
PMIX_DISK_STATS
PMIX_ERR_JOB_EXE_NOT_FOUND
PMIX_ERR_JOB_INSUFFICIENT_RESOURCES
PMIX_ERR_JOB_SYS_OP_FAILED
PMIX_ERR_JOB_WDIR_NOT_FOUND
...
OpenPMIx v4.1.0 does *not* include support for the storage definitions in the PMIx v4.1 Standard, and removed support for one constant. This includes the following:
