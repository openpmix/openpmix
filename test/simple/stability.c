...
int main(int argc, char **argv)
{
    ...
    for (n = 1; n < argc; n++) {
        if (0 == strcmp("-n", argv[n]) && NULL != argv[n + 1]) {
        ...
        } else if ((0 == strcmp("-sleep", argv[n]) || 0 == strcmp("--sleep", argv[n]))
                   && NULL != argv[n + 1]) {
            ...
            exit(0);
        } else if (0 == strcmp("-net-test", argv[n]) || 0 == strcmp("--net-test", argv[n])) {
            /* test network support */
            nettest = true;
        } else if (0 == strcmp("-arrays", argv[n]) || 0 == strcmp("--arrays", argv[n])) {
            /* test network support */
            arrays = true;
        }
    }
                              ...
                              pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t rc;
    myxfer_t *x;
    PMIX_HIDE_UNUSED_PARAMS(server_object, msg, procs, nprocs);
...
{
    myxfer_t *x;
    PMIX_HIDE_UNUSED_PARAMS(proc, job_info, ninfo, apps, napps);
