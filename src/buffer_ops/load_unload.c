...
int pmix_bfrop_load(pmix_buffer_t *buffer, void *payload,
                  int32_t bytes_used)
{
...
int pmix_bfrop_copy_payload(pmix_buffer_t *dest, pmix_buffer_t *src)
{
    ...
    if (NULL == (dst_ptr = pmix_bfrop_buffer_extend(dest, bytes_left))) {
...
int pmix_value_load(pmix_value_t *kv,
...
int pmix_value_unload(pmix_value_t *kv,
                      void **data, pmix_data_type_t type)
{
    pmix_byte_object_t *boptr;
