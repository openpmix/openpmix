...
#    define PMIX_ATOMIC_DEFINE_CMPXCG_OP(type, bits, operation, name)                              \
        ...
        {                                                                                          \
            ...
            do {                                                                                   \
                oldval = *addr;                                                                    \
            } while (!pmix_atomic_compare_exchange_strong_##bits(addr, &oldval,                    \
...
#        define PMIX_ATOMIC_DEFINE_CMPXCG_XX(semantics)                                            \
            static inline bool pmix_atomic_compare_exchange_strong##semantics##xx(                 \
                pmix_atomic_intptr_t *addr, intptr_t *oldval, int64_t newval, const size_t length) \
            {                                                                                      \
                switch (length) {                                                                  \
                case 4:                                                                            \
...
#        define PMIX_ATOMIC_DEFINE_CMPXCG_PTR_XX(semantics)                                     \
            static inline bool                                                                  \
                pmix_atomic_compare_exchange_strong##semantics##ptr(pmix_atomic_intptr_t *addr, \
                                                                    intptr_t *oldval,           \
                                                                    intptr_t newval)            \
            {                                                                                   \
                return pmix_atomic_compare_exchange_strong_32((pmix_atomic_int32_t *) addr,     \
...
#        define pmix_atomic_swap_ptr(addr, value) \
            (intptr_t) pmix_atomic_swap_32((pmix_atomic_int32_t *) addr, (int32_t) value)
#    elif SIZEOF_VOID_P == 8 && PMIX_HAVE_ATOMIC_SWAP_64
#        define pmix_atomic_swap_ptr(addr, value) \
            (intptr_t) pmix_atomic_swap_64((pmix_atomic_int64_t *) addr, (int64_t) value)
#    endif
...
#        define pmix_atomic_sc_ptr(addr, value, ret) \
            pmix_atomic_sc_32((pmix_atomic_int32_t *) (addr), (intptr_t)(value), ret)
...
#        define pmix_atomic_sc_ptr(addr, value, ret) \
            pmix_atomic_sc_64((pmix_atomic_int64_t *) (addr), (intptr_t)(value), ret)
...
#    define PMIX_ATOMIC_DEFINE_OP_FETCH(op, operation, type, ptr_type, suffix)             \
        static inline type pmix_atomic_##op##_fetch_##suffix(pmix_atomic_##ptr_type *addr, \
                                                             type value)                   \
        {                                                                                  \
...
static inline int pmix_atomic_trylock(pmix_atomic_lock_t *lock)
{
    ...
    bool ret = pmix_atomic_compare_exchange_strong_acq_32(&lock->u.lock, &unlocked,
                                                          PMIX_ATOMIC_LOCK_LOCKED);
