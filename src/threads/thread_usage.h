...
#    define PMIX_THREAD_DEFINE_ATOMIC_OP(type, name, operator, suffix)                \
        __pmix_attribute_always_inline__ static __inline__ type                       \
            pmix_thread_##name##_fetch_##suffix(pmix_atomic_##type *addr, type delta) \
        {                                                                             \
            return pmix_atomic_##name##_fetch_##suffix(addr, delta);                  \
        }                                                                             \
                                                                                      \
        __pmix_attribute_always_inline__ static __inline__ type                       \
            pmix_thread_fetch_##name##_##suffix(pmix_atomic_##type *addr, type delta) \
        {                                                                             \
...
#    define PMIX_THREAD_DEFINE_ATOMIC_COMPARE_EXCHANGE(type, addr_type, suffix)              \
        __pmix_attribute_always_inline__ static __inline__ bool                              \
            pmix_thread_compare_exchange_strong_##suffix(pmix_atomic_##addr_type *addr,      \
                                                         type *compare, type value)          \
        {                                                                                    \
            return pmix_atomic_compare_exchange_strong_##suffix(addr, (addr_type *) compare, \
                                                                (addr_type) value);          \
        }

#    define PMIX_THREAD_DEFINE_ATOMIC_SWAP(type, addr_type, suffix)                \
        __pmix_attribute_always_inline__ static __inline__ type                    \
            pmix_thread_swap_##suffix(pmix_atomic_##addr_type *ptr, type newvalue) \
        {                                                                          \
...
#    define PMIX_THREAD_COMPARE_EXCHANGE_STRONG_PTR(x, y, z)                                \
        pmix_thread_compare_exchange_strong_ptr((pmix_atomic_intptr_t *) x, (intptr_t *) y, \
                                                (intptr_t) z)
...
#    define PMIX_THREAD_SWAP_PTR(x, y) \
        pmix_thread_swap_ptr((pmix_atomic_intptr_t *) x, (intptr_t) y)
