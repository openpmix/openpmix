...
static pmix_status_t nvd_init(void);
static void nvd_finalize(void);
...
static void encon(nvd_envar_t *p)
{
    PMIX_ENVAR_CONSTRUCT(&p->envar);
}
static void endes(nvd_envar_t *p)
{
    PMIX_ENVAR_DESTRUCT(&p->envar);
}
static PMIX_CLASS_INSTANCE(nvd_envar_t, pmix_list_item_t, encon, endes);
...
static void nscon(nvd_nspace_t *p)
{
    PMIX_CONSTRUCT(&p->envars, pmix_list_t);
}
static void nsdes(nvd_nspace_t *p)
{
    PMIX_LIST_DESTRUCT(&p->envars);
}
static PMIX_CLASS_INSTANCE(nvd_nspace_t, pmix_list_item_t, nscon, nsdes);
...
static pmix_status_t setup_local_network(pmix_namespace_t *nptr, pmix_info_t info[], size_t ninfo)
{
    ...
    for (n = 0; n < ninfo; n++) {
        ...
        if (PMIX_CHECK_KEY(&info[n], PMIX_PNET_NVD_BLOB)) {
            pmix_output_verbose(2, pmix_pnet_base_framework.framework_output,
                                "pnet:nvd:setup_local_network found my blob");
            ...
            ns = NULL;
            PMIX_LIST_FOREACH (n2, &mynspaces, nvd_nspace_t) {
...
static pmix_status_t setup_fork(pmix_namespace_t *nptr, const pmix_proc_t *proc, char ***env)
{
    ...
    ns = NULL;
    PMIX_LIST_FOREACH (ns2, &mynspaces, nvd_nspace_t) {
    ...
    if (NULL == ns) {
         ...
         * it doesn't have any nvd data */
        return PMIX_ERR_TAKE_NEXT_OPTION;
    }

    PMIX_LIST_FOREACH (ev, &ns->envars, nvd_envar_t) {
