...
static pmix_status_t usnic_init(void);
static void usnic_finalize(void);
...
pmix_pnet_module_t pmix_usnic_module = {.name = "usnic",
                                      .init = usnic_init,
                                      .finalize = usnic_finalize,
                                      .allocate = allocate,
...
static void encon(usnic_envar_t *p)
{
    PMIX_ENVAR_CONSTRUCT(&p->envar);
}
static void endes(usnic_envar_t *p)
{
    PMIX_ENVAR_DESTRUCT(&p->envar);
}
static PMIX_CLASS_INSTANCE(usnic_envar_t, pmix_list_item_t, encon, endes);
...
static void nscon(usnic_nspace_t *p)
{
    PMIX_CONSTRUCT(&p->envars, pmix_list_t);
}
static void nsdes(usnic_nspace_t *p)
{
    PMIX_LIST_DESTRUCT(&p->envars);
}
static PMIX_CLASS_INSTANCE(usnic_nspace_t, pmix_list_item_t, nscon, nsdes);
...
static pmix_status_t setup_local_network(pmix_namespace_t *nptr, pmix_info_t info[], size_t ninfo)
{
    ...
    for (n = 0; n < ninfo; n++) {
        ...
        if (PMIX_CHECK_KEY(&info[n], PMIX_PNET_USNIC_BLOB)) {
            pmix_output_verbose(2, pmix_pnet_base_framework.framework_output,
                                "pnet:usnic:setup_local_network found my blob");
            ...
            ns = NULL;
            PMIX_LIST_FOREACH (n2, &mynspaces, usnic_nspace_t) {
...
static pmix_status_t setup_fork(pmix_namespace_t *nptr, const pmix_proc_t *proc, char ***env)
{
    usnic_nspace_t *ns, *ns2;
    usnic_envar_t *ev;

    pmix_output_verbose(2, pmix_pnet_base_framework.framework_output, "pnet:usnic: setup fork for %s",
                        PMIX_NAME_PRINT(proc));

    /* see if we already have this nspace */
    ns = NULL;
    PMIX_LIST_FOREACH (ns2, &mynspaces, usnic_nspace_t) {
    ...
    if (NULL == ns) {
         ...
         * it doesn't have any usnic data */
        return PMIX_ERR_TAKE_NEXT_OPTION;
    }

    PMIX_LIST_FOREACH (ev, &ns->envars, usnic_envar_t) {
