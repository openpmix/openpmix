 ...
 */
 PMIX_EXPORT pmix_ptl_server_component_t mca_ptl_server_component = {
    .super = {
        .base = {
...
static pmix_status_t component_open(void)
{
    ...
    memset(&mca_ptl_server_component.connection, 0, sizeof(mca_ptl_server_component.connection));

    /* check for environ-based directives
     * on system tmpdir to use */
    if (PMIX_PEER_IS_SERVER(pmix_globals.mypeer) ||
        PMIX_PEER_IS_LAUNCHER(pmix_globals.mypeer)) {
        mca_ptl_server_component.session_tmpdir = strdup(pmix_server_globals.tmpdir);
    } else {
        if (NULL != (tdir = getenv("PMIX_SERVER_TMPDIR"))) {
            mca_ptl_server_component.session_tmpdir = strdup(tdir);
        } else {
            mca_ptl_server_component.session_tmpdir = strdup(pmix_tmp_directory());
        }
    }

    if (PMIX_PEER_IS_SERVER(pmix_globals.mypeer) ||
        PMIX_PEER_IS_LAUNCHER(pmix_globals.mypeer)) {
        mca_ptl_server_component.system_tmpdir = strdup(pmix_server_globals.system_tmpdir);
    } else {
        if (NULL != (tdir = getenv("PMIX_SYSTEM_TMPDIR"))) {
            mca_ptl_server_component.system_tmpdir = strdup(tdir);
        } else {
            mca_ptl_server_component.system_tmpdir = strdup(pmix_tmp_directory());
        }
    }

    if (NULL != mca_ptl_server_component.report_uri &&
        0 != strcmp(mca_ptl_server_component.report_uri, "-") &&
        0 != strcmp(mca_ptl_server_component.report_uri, "+")) {
        urifile = strdup(mca_ptl_server_component.report_uri);
    }

    if (PMIX_PEER_IS_SERVER(pmix_globals.mypeer) ||
        PMIX_PEER_IS_LAUNCHER(pmix_globals.mypeer)) {
        if (NULL != (tdir = getenv("PMIX_LAUNCHER_RENDEZVOUS_FILE"))) {
            mca_ptl_server_component.rendezvous_filename = strdup(tdir);
        }
    }
...
{
    if (NULL != mca_ptl_server_component.system_filename) {
        if (created_system_filename) {
            remove(mca_ptl_server_component.system_filename);
        }
        free(mca_ptl_server_component.system_filename);
    }
    if (NULL != mca_ptl_server_component.session_filename) {
        if (created_session_filename) {
            remove(mca_ptl_server_component.session_filename);
        }
        free(mca_ptl_server_component.session_filename);
    }
    if (NULL != mca_ptl_server_component.nspace_filename) {
        if (created_nspace_filename) {
            remove(mca_ptl_server_component.nspace_filename);
        }
        free(mca_ptl_server_component.nspace_filename);
    }
    if (NULL != mca_ptl_server_component.pid_filename) {
        if (created_pid_filename) {
            remove(mca_ptl_server_component.pid_filename);
        }
        free(mca_ptl_server_component.pid_filename);
    }
    if (NULL != mca_ptl_server_component.rendezvous_filename) {
        if (created_rendezvous_file) {
            remove(mca_ptl_server_component.rendezvous_filename);
        }
        free(mca_ptl_server_component.rendezvous_filename);
    }
    if (NULL != urifile) {
        if (created_urifile) {
            /* remove the file */
    ...
    if (NULL != mca_ptl_server_component.session_tmpdir) {
        /* if I am a tool or a server, then remove my session directory if empty */
        if (created_session_tmpdir) {
            pmix_os_dirpath_destroy(mca_ptl_server_component.session_tmpdir,
                                    true, NULL);
        }
        free(mca_ptl_server_component.session_tmpdir);
    }
    if (NULL != mca_ptl_server_component.system_tmpdir) {
        if (created_system_tmpdir) {
            pmix_os_dirpath_destroy(mca_ptl_server_component.system_tmpdir,
                                    true, NULL);
        }
        free(mca_ptl_server_component.system_tmpdir);
    }
    return PMIX_SUCCESS;
}
...
static int component_query(pmix_mca_base_module_t **module, int *priority)
{
    /* if we are not a server, then don't take us */
    if (!PMIX_PEER_IS_SERVER(pmix_globals.mypeer)) {
...
static pmix_status_t setup_fork(const pmix_proc_t *proc, char ***env)
{
    pmix_setenv("PMIX_SERVER_TMPDIR", mca_ptl_server_component.session_tmpdir, true, env);
    pmix_setenv("PMIX_SYSTEM_TMPDIR", mca_ptl_server_component.system_tmpdir, true, env);
                                    ...
                                    bool *need_listener)
{
    ...
    pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                        "ptl:server setup_listener");

    /* scan the info keys and process any override instructions */
    if (NULL != info) {
        for (n=0; n < ninfo; n++) {
            if (PMIX_CHECK_KEY(&info[n], PMIX_TCP_IF_INCLUDE)) {
                if (NULL != mca_ptl_server_component.if_include) {
                    free(mca_ptl_server_component.if_include);
                }
                mca_ptl_server_component.if_include = strdup(info[n].value.data.string);
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_TCP_IF_EXCLUDE)) {
                if (NULL != mca_ptl_server_component.if_exclude) {
                    free(mca_ptl_server_component.if_exclude);
                }
                mca_ptl_server_component.if_exclude = strdup(info[n].value.data.string);
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_TCP_IPV4_PORT)) {
                mca_ptl_server_component.ipv4_port = info[n].value.data.integer;
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_TCP_IPV6_PORT)) {
                mca_ptl_server_component.ipv6_port = info[n].value.data.integer;
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_TCP_DISABLE_IPV4)) {
                mca_ptl_server_component.disable_ipv4_family = PMIX_INFO_TRUE(&info[n]);
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_TCP_DISABLE_IPV6)) {
                mca_ptl_server_component.disable_ipv6_family = PMIX_INFO_TRUE(&info[n]);
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_SERVER_REMOTE_CONNECTIONS)) {
                mca_ptl_server_component.remote_connections = PMIX_INFO_TRUE(&info[n]);
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_SERVER_URI)) {
                if (NULL != mca_ptl_server_component.super.uri) {
                    free(mca_ptl_server_component.super.uri);
                }
                mca_ptl_server_component.super.uri = strdup(info[n].value.data.string);
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_TCP_REPORT_URI)) {
                if (NULL != mca_ptl_server_component.report_uri) {
                    free(mca_ptl_server_component.report_uri);
                }
                mca_ptl_server_component.report_uri = strdup(info[n].value.data.string);
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_SERVER_TMPDIR)) {
                if (NULL != mca_ptl_server_component.session_tmpdir) {
                    free(mca_ptl_server_component.session_tmpdir);
                }
                mca_ptl_server_component.session_tmpdir = strdup(info[n].value.data.string);
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_SYSTEM_TMPDIR)) {
                if (NULL != mca_ptl_server_component.system_tmpdir) {
                    free(mca_ptl_server_component.system_tmpdir);
                }
                mca_ptl_server_component.system_tmpdir = strdup(info[n].value.data.string);
                ...
                tool_support = PMIX_INFO_TRUE(&info[n]);
            } else if (PMIX_PEER_IS_LAUNCHER(pmix_globals.mypeer) &&
                       PMIX_CHECK_KEY(&info[n], PMIX_LAUNCHER_RENDEZVOUS_FILE)) {
                if (NULL != mca_ptl_server_component.rendezvous_filename) {
                    free(mca_ptl_server_component.rendezvous_filename);
                }
                mca_ptl_server_component.rendezvous_filename = strdup(info[n].value.data.string);
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_CONNECT_MAX_RETRIES)) {
                 PMIX_VALUE_GET_NUMBER(rc, &info[n].value, mca_ptl_server_component.max_retries, int);
                 if (PMIX_SUCCESS != rc) {
                    return rc;
                 }
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_CONNECT_RETRY_DELAY)) {
                 PMIX_VALUE_GET_NUMBER(rc, &info[n].value, mca_ptl_server_component.wait_to_connect, int);
                 if (PMIX_SUCCESS != rc) {
                    return rc;
                 }
            }
     ...
     */
    if (NULL != mca_ptl_server_component.if_include) {
        interfaces = split_and_resolve(&mca_ptl_server_component.if_include,
                                       "include");
        including = true;
    } else if (NULL != mca_ptl_server_component.if_exclude) {
        interfaces = split_and_resolve(&mca_ptl_server_component.if_exclude,
                                       "exclude");
        including = false;
    }
    ...
    for (i = pmix_ifbegin(); i >= 0; i = pmix_ifnext(i)) {
        if (PMIX_SUCCESS != pmix_ifindextoaddr(i, (struct sockaddr*)&my_ss, sizeof(my_ss))) {
            pmix_output (0, "ptl_server: problems getting address for index %i (kernel index %i)\n",
        ...
        if (AF_INET == my_ss.ss_family &&
            mca_ptl_server_component.disable_ipv4_family) {
            continue;
        } else if (AF_INET6 == my_ss.ss_family &&
                   mca_ptl_server_component.disable_ipv6_family) {
            continue;
        }
        ...
        if (NULL != interfaces) {
            ...
            if (PMIX_ERR_FABRIC_NOT_PARSEABLE == rc) {
                pmix_show_help("help-ptl-server.txt", "not-parseable", true);
            ...
            if (including) {
                if (PMIX_SUCCESS != rc) {
                    pmix_output_verbose(10, pmix_ptl_base_framework.framework_output,
                                        "ptl:server:init rejecting interface %s (not in include list)", name);
                    continue;
                }
            } else {
                ...
                if (PMIX_SUCCESS == rc) {
                    pmix_output_verbose(10, pmix_ptl_base_framework.framework_output,
                                        "ptl:server:init rejecting interface %s (in exclude list)", name);
                    continue;
                }
            }
        }
        ...
        if (pmix_ifisloopback(i)) {
            if (mca_ptl_server_component.remote_connections) {
                /* ignore loopback */
                continue;
            } else {
                pmix_output_verbose(5, pmix_ptl_base_framework.framework_output,
                                    "ptl:server:init loopback interface %s selected", name);
                saveindex = i;
                break;
            }
        } else {
    ...
    if (PMIX_SUCCESS != pmix_ifindextoaddr(saveindex,
                                           (struct sockaddr*)&mca_ptl_server_component.connection,
                                           sizeof(struct sockaddr))) {
        pmix_output (0, "ptl:server: problems getting address for kernel index %i\n",
                     pmix_ifindextokindex(saveindex));
        return PMIX_ERR_NOT_AVAILABLE;
    }

    /* set the port */
    if (AF_INET == mca_ptl_server_component.connection.ss_family) {
        ((struct sockaddr_in*) &mca_ptl_server_component.connection)->sin_port = htons(mca_ptl_server_component.ipv4_port);
        if (0 != mca_ptl_server_component.ipv4_port) {
            flags = 1;
        }
    } else if (AF_INET6 == mca_ptl_server_component.connection.ss_family) {
        ((struct sockaddr_in6*) &mca_ptl_server_component.connection)->sin6_port = htons(mca_ptl_server_component.ipv6_port);
        if (0 != mca_ptl_server_component.ipv6_port) {
            flags = 1;
        }
    }
    ...
    lt->ptl = (struct pmix_ptl_module_t*)&pmix_ptl_server_module;
    lt->cbfunc = connection_handler;

    addrlen = sizeof(struct sockaddr_storage);
    /* create a listen socket for incoming connection attempts */
    lt->socket = socket(mca_ptl_server_component.connection.ss_family, SOCK_STREAM, 0);
    if (lt->socket < 0) {
    ...
    if (setsockopt (lt->socket, SOL_SOCKET, SO_REUSEADDR, (const char *)&flags, sizeof(flags)) < 0) {
        pmix_output(0, "ptl:server:create_listen: unable to set the "
    ...
    if (bind(lt->socket, (struct sockaddr*)&mca_ptl_server_component.connection, sizeof(struct sockaddr)) < 0) {
    ...
    if (getsockname(lt->socket, (struct sockaddr*)&mca_ptl_server_component.connection, &addrlen) < 0) {
        pmix_output(0, "ptl:server:create_listen: getsockname(): %s (%d)",
                    strerror(pmix_socket_errno), pmix_socket_errno);
        CLOSE_THE_SOCKET(lt->socket);
        goto sockerror;
    }
    ...
    if (AF_INET == mca_ptl_server_component.connection.ss_family) {
        prefix = "tcp4://";
        myport = ntohs(((struct sockaddr_in*) &mca_ptl_server_component.connection)->sin_port);
        inet_ntop(AF_INET, &((struct sockaddr_in*) &mca_ptl_server_component.connection)->sin_addr,
                  myconnhost, PMIX_MAXHOSTNAMELEN-1);
    } else if (AF_INET6 == mca_ptl_server_component.connection.ss_family) {
        prefix = "tcp6://";
        myport = ntohs(((struct sockaddr_in6*) &mca_ptl_server_component.connection)->sin6_port);
        inet_ntop(AF_INET6, &((struct sockaddr_in6*) &mca_ptl_server_component.connection)->sin6_addr,
                  myconnhost, PMIX_MAXHOSTNAMELEN-1);
    } else {
        goto sockerror;
    }
    ...
    if (NULL != mca_ptl_server_component.report_uri) {
        ...
        if (0 == strcmp(mca_ptl_server_component.report_uri, "-")) {
            fprintf(stdout, "%s\n", lt->uri);
        } else if (0 == strcmp(mca_ptl_server_component.report_uri, "+")) {
            /* output to stderr */
            fprintf(stderr, "%s\n", lt->uri);
        } else {
            /* must be a file */
            FILE *fp;
            fp = fopen(mca_ptl_server_component.report_uri, "w");
            if (NULL == fp) {
                pmix_output(0, "Impossible to open the file %s in write mode\n", mca_ptl_server_component.report_uri);
                PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
                CLOSE_THE_SOCKET(lt->socket);
                free(mca_ptl_server_component.system_filename);
                mca_ptl_server_component.system_filename = NULL;
                goto sockerror;
            }
    ...
    if (NULL != mca_ptl_server_component.rendezvous_filename) {
        ...
        if (PMIX_PEER_IS_TOOL(pmix_globals.mypeer)) {
            struct stat buf;
            /* coverity[toctou] */
            if (0 == stat(mca_ptl_server_component.rendezvous_filename, &buf)) {
                goto nextstep;
            }
        }
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "WRITING RENDEZVOUS FILE %s",
                            mca_ptl_server_component.rendezvous_filename);
        fp = fopen(mca_ptl_server_component.rendezvous_filename, "w");
        if (NULL == fp) {
            pmix_output(0, "Impossible to open the file %s in write mode\n", mca_ptl_server_component.rendezvous_filename);
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
            free(mca_ptl_server_component.rendezvous_filename);
            mca_ptl_server_component.rendezvous_filename = NULL;
            goto sockerror;
        }
        ...
        fclose(fp);
        /* set the file mode */
        if (0 != chmod(mca_ptl_server_component.rendezvous_filename, S_IRUSR | S_IWUSR | S_IRGRP)) {
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
            free(mca_ptl_server_component.rendezvous_filename);
            mca_ptl_server_component.rendezvous_filename = NULL;
  ...
  nextstep:
    ...
    if (system_tool) {
        if (0 == stat(mca_ptl_server_component.system_tmpdir, &sbuf)) {
            /* already exists - check if it is a directory */
            if (! S_ISDIR(sbuf.st_mode)) {
                /* nope - we are hosed */
                pmix_output(0, "System tmpdir %s is not a directory\n", mca_ptl_server_component.system_tmpdir);
                PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
                CLOSE_THE_SOCKET(lt->socket);
                goto sockerror;
            }
            ...
            /* need to create it */
            rc = mkdir(mca_ptl_server_component.system_tmpdir, 0755);
            if (0 != rc) {
        ...
        if (0 > asprintf(&mca_ptl_server_component.system_filename, "%s/pmix.sys.%s",
                         mca_ptl_server_component.system_tmpdir, myhost)) {
            CLOSE_THE_SOCKET(lt->socket);
            goto sockerror;
        }
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "WRITING SYSTEM FILE %s",
                            mca_ptl_server_component.system_filename);
        fp = fopen(mca_ptl_server_component.system_filename, "w");
        if (NULL == fp) {
            pmix_output(0, "Impossible to open the file %s in write mode\n", mca_ptl_server_component.system_filename);
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
        ...
        fclose(fp);
        /* set the file mode */
        if (0 != chmod(mca_ptl_server_component.system_filename, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)) {
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
            free(mca_ptl_server_component.system_filename);
            mca_ptl_server_component.system_filename = NULL;
    ...
    if (session_tool) {
        if (0 == stat(mca_ptl_server_component.session_tmpdir, &sbuf)) {
            /* already exists - check if it is a directory */
            if (! S_ISDIR(sbuf.st_mode)) {
                /* nope - we are hosed */
                pmix_output(0, "Session tmpdir %s is not a directory\n", mca_ptl_server_component.session_tmpdir);
                PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
                CLOSE_THE_SOCKET(lt->socket);
                goto sockerror;
            }
            ...
            /* need to create it */
            rc = mkdir(mca_ptl_server_component.session_tmpdir, 0755);
            if (0 != rc) {
        ...
        if (0 > asprintf(&mca_ptl_server_component.session_filename, "%s/pmix.%s.tool",
                         mca_ptl_server_component.session_tmpdir, myhost)) {
            CLOSE_THE_SOCKET(lt->socket);
            goto sockerror;
        }
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "WRITING SESSION TOOL FILE %s",
                            mca_ptl_server_component.session_filename);
        fp = fopen(mca_ptl_server_component.session_filename, "w");
        if (NULL == fp) {
            pmix_output(0, "Impossible to open the file %s in write mode\n", mca_ptl_server_component.session_filename);
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
            free(mca_ptl_server_component.session_filename);
            mca_ptl_server_component.session_filename = NULL;
            goto sockerror;
        }
        ...
        fclose(fp);
        /* set the file mode */
        if (0 != chmod(mca_ptl_server_component.session_filename, S_IRUSR | S_IWUSR | S_IRGRP)) {
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
            free(mca_ptl_server_component.session_filename);
            mca_ptl_server_component.session_filename = NULL;
    ...
    if (tool_support) {
        ...
        mypid = getpid();
        if (0 > asprintf(&mca_ptl_server_component.pid_filename, "%s/pmix.%s.tool.%d",
                         mca_ptl_server_component.session_tmpdir, myhost, mypid)) {
            CLOSE_THE_SOCKET(lt->socket);
            goto sockerror;
        }
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "WRITING PID TOOL FILE %s",
                            mca_ptl_server_component.pid_filename);
        fp = fopen(mca_ptl_server_component.pid_filename, "w");
        if (NULL == fp) {
            pmix_output(0, "Impossible to open the file %s in write mode\n", mca_ptl_server_component.pid_filename);
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
            free(mca_ptl_server_component.pid_filename);
            mca_ptl_server_component.pid_filename = NULL;
            goto sockerror;
        }
        ...
        fclose(fp);
        /* set the file mode */
        if (0 != chmod(mca_ptl_server_component.pid_filename, S_IRUSR | S_IWUSR | S_IRGRP)) {
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
            free(mca_ptl_server_component.pid_filename);
            mca_ptl_server_component.pid_filename = NULL;
        ...
        if (0 > asprintf(&mca_ptl_server_component.nspace_filename, "%s/pmix.%s.tool.%s",
                         mca_ptl_server_component.session_tmpdir, myhost, pmix_globals.myid.nspace)) {
            CLOSE_THE_SOCKET(lt->socket);
            goto sockerror;
        }
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "WRITING NSPACE TOOL FILE %s",
                            mca_ptl_server_component.nspace_filename);
        fp = fopen(mca_ptl_server_component.nspace_filename, "w");
        if (NULL == fp) {
            pmix_output(0, "Impossible to open the file %s in write mode\n", mca_ptl_server_component.nspace_filename);
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
            free(mca_ptl_server_component.nspace_filename);
            mca_ptl_server_component.nspace_filename = NULL;
            goto sockerror;
        }
        ...
        fclose(fp);
        /* set the file mode */
        if (0 != chmod(mca_ptl_server_component.nspace_filename, S_IRUSR | S_IWUSR | S_IRGRP)) {
            PMIX_ERROR_LOG(PMIX_ERR_FILE_OPEN_FAILURE);
            CLOSE_THE_SOCKET(lt->socket);
            free(mca_ptl_server_component.nspace_filename);
            mca_ptl_server_component.nspace_filename = NULL;
    ...
    if (PMIX_PEER_IS_TOOL(pmix_globals.mypeer) && pmix_globals.connected) {
        ...
        pmix_info_t dir;
        if (NULL != mca_ptl_server_component.nspace_filename) {
            pmix_argv_append_nosize(&clnup, mca_ptl_server_component.nspace_filename);
        }
        if (NULL != mca_ptl_server_component.session_filename) {
            pmix_argv_append_nosize(&clnup, mca_ptl_server_component.session_filename);
        }
        if (NULL != clnup) {
...
static char **split_and_resolve(char **orig_str, char *name)
{
    ...
    for (save = i = 0; NULL != argv[i]; ++i) {
        ...
        str = strchr(argv[i], '/');
        if (NULL == str) {
            pmix_show_help("help-ptl-server.txt", "invalid if_inexclude",
        ...
        *str = '\0';
        ...
        pmix_output_verbose(20, pmix_ptl_base_framework.framework_output,
                            "ptl:server: Searching for %s address+prefix: %s / %u",
                            name,
...
static void connection_handler(int sd, short args, void *cbdata)
{
    ...
    pmix_output_verbose(8, pmix_ptl_base_framework.framework_output,
                        "ptl:server:connection_handler: new connection: %d",
                        pnd->sd);
    ...
    if (PMIX_SUCCESS != (rc = pmix_ptl_base_recv_blocking(pnd->sd, msg, hdr.nbytes))) {
        ...
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "ptl:server:connection_handler unable to complete recv of connect-ack with client ON SOCKET %d",
                            pnd->sd);
        free(msg);
        CLOSE_THE_SOCKET(pnd->sd);
        PMIX_RELEASE(pnd);
    ...
    if (0 == pnd->flag) {
        ...
        /* they are a tool */
        PMIX_SET_PROC_TYPE(&proc_type, PMIX_PROC_TOOL);
        /* extract the uid/gid */
        if (sizeof(uint32_t) <= cnt) {
            memcpy(&u32, mg, sizeof(uint32_t));
            mg += sizeof(uint32_t);
        ...
        /* they are a launcher */
        PMIX_SET_PROC_TYPE(&proc_type, PMIX_PROC_LAUNCHER);
        /* extract the uid/gid */
        if (sizeof(uint32_t) <= cnt) {
            memcpy(&u32, mg, sizeof(uint32_t));
            mg += sizeof(uint32_t);
    ...
    peer->nptr->compat.ptl = &pmix_ptl_server_module;
                        ...
                        pmix_proc_t *proc, void *cbdata)
{
    ...
    pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                        "pmix:server:cnct_cbfunc returning %s:%d %s",
