                                     ...
                                     pmix_info_t *info, size_t ninfo)
{
    ...
    pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                        "ptl:server: connecting to server");
    ...
    if (NULL != info) {
        for (n=0; n < ninfo; n++) {
            if (PMIX_CHECK_KEY(&info[n], PMIX_CONNECT_TO_SYSTEM)) {
                ...
                mca_ptl_server_component.wait_to_connect = info[n].value.data.uint32;
            } else if (PMIX_CHECK_KEY(&info[n], PMIX_CONNECT_MAX_RETRIES)) {
                mca_ptl_server_component.max_retries = info[n].value.data.uint32;
    ...
    if (NULL == suri && !reconnect && NULL != mca_ptl_server_component.super.uri) {
        suri = strdup(mca_ptl_server_component.super.uri);
    }
    ...
    if (NULL != suri) {
        ...
        if (0 == strncmp(suri, "file:", 5)) {
            pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                                "ptl:server:tool getting connection info from %s", suri);
            nspace = NULL;
        ...
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "ptl:server:tool attempt connect using given URI %s", suri);
    ...
    if (NULL != rendfile) {
        ...
        if (PMIX_SUCCESS == rc) {
            pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                                "ptl:server:tool attempt connect to rendezvous server at %s", suri);
            /* go ahead and try to connect */
    ...
    if (system_level || system_level_only) {
        if (0 > asprintf(&filename, "%s/pmix.sys.%s", mca_ptl_server_component.system_tmpdir, myhost)) {
            rc = PMIX_ERR_NOMEM;
            goto cleanup;
        }
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "ptl:server:tool looking for system server at %s",
                            filename);
        /* try to read the file */
        ...
        if (PMIX_SUCCESS == rc) {
            pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                                "ptl:server:tool attempt connect to system server at %s", suri);
            /* go ahead and try to connect */
            if (PMIX_SUCCESS == try_connect(suri, &sd, iptr, niptr)) {
                /* don't free nspace - we will use it below */
                goto complete;
    ...
    if (system_level_only) {
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "ptl:server: connecting to system failed");
        rc = PMIX_ERR_UNREACH;
        goto cleanup;
    }
    ...
    if (0 != pid) {
        ...
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "ptl:server:tool searching for given session server %s",
                            filename);
        nspace = NULL;
        rc = df_search(mca_ptl_server_component.system_tmpdir,
    ...
    pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                        "ptl:server:tool searching for session server %s",
                        filename);
    nspace = NULL;
    rc = df_search(mca_ptl_server_component.system_tmpdir,
                   filename, iptr, niptr, &sd, &nspace, &rank, &suri, peer);
  ...
  complete:
    pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                        "server_peer_try_connect: Connection across to server succeeded");
    ...
    if (0 == access(filename, R_OK)) {
        ...
        if (ENOENT == errno) {
            ...
            do {
                ...
                if (0 < mca_ptl_server_component.wait_to_connect) {
                    tv.tv_sec = mca_ptl_server_component.wait_to_connect;
                    tv.tv_usec = 0;
            ...
            } while (retries < mca_ptl_server_component.max_retries);
...
static pmix_status_t try_connect(char *uri, int *sd, pmix_info_t iptr[], size_t niptr)
{
    ...
    pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                        "pmix:server try connect to %s", uri);

    /* mark that we are the active module for this server */
    pmix_client_globals.myserver->nptr->compat.ptl = &pmix_ptl_server_module;

    /* setup the path to the daemon rendezvous point */
    memset(&mca_ptl_server_component.connection, 0, sizeof(struct sockaddr_storage));
    if (0 == strncmp(uri, "server4", 4)) {
        /* need to skip the server4: part */
        p = strdup(&uri[7]);
        if (NULL == p) {
        ...
        *p2 = '\0';
        ...
        host = p;
        /* load the address */
        in = (struct sockaddr_in*)&mca_ptl_server_component.connection;
        ...
        *p2 = '\0';
        ...
        /* load the address */
        in6 = (struct sockaddr_in6*)&mca_ptl_server_component.connection;
        in6->sin6_family = AF_INET6;
        if (0 == inet_pton(AF_INET6, host, (void*)&in6->sin6_addr)) {
            pmix_output (0, "ptl_server_parse_uri: Could not convert %s\n", host);
            free(p);
            PMIX_ERROR_LOG(PMIX_ERR_BAD_PARAM);
            return PMIX_ERR_BAD_PARAM;
        }
  ...
  retry:
    ...
    if (PMIX_SUCCESS != (rc = pmix_ptl_base_connect(&mca_ptl_server_component.connection, len, sd))) {
    ...
    if (PMIX_SUCCESS != (rc = recv_connect_ack(*sd, myflag))) {
        ...
        if (PMIX_ERR_TEMP_UNAVAILABLE == rc) {
            ++retries;
            if( retries < mca_ptl_server_component.handshake_max_retries ) {
                goto retry;
            }
        }
        return rc;
...
static pmix_status_t recv_connect_ack(int sd, uint8_t myflag)
{
    ...
    if (0 != getsockopt(sd, SOL_SOCKET, SO_RCVTIMEO, (void*)&save, &sz)) {
        ...
        tv.tv_sec  = mca_ptl_server_component.handshake_wait_time;
        tv.tv_usec = 0;
...
#if defined(SERVER_NODELAY)
    int optval;
    optval = 1;
    if (setsockopt(sd, IPPROTO_SERVER, SERVER_NODELAY, (char *)&optval, sizeof(optval)) < 0) {
        opal_backtrace_print(stderr, NULL, 1);
        pmix_output_verbose(5, pmix_ptl_base_framework.framework_output,
                            "[%s:%d] setsockopt(SERVER_NODELAY) failed: %s (%d)",
                            __FILE__, __LINE__,
                               ...
                               pmix_peer_t *peer)
{
    ...
    pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                        "pmix:server: searching directory %s", dirname);
    ...
    while (NULL != (dir_entry = readdir(cur_dirp))) {
        ...
        pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                            "pmix:server: checking %s vs %s", dir_entry->d_name, prefix);
        /* see if it starts with our prefix */
        if (0 == strncmp(dir_entry->d_name, prefix, strlen(prefix))) {
            /* try to read this file */
            pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                                "pmix:server: reading file %s", newdir);
            rc = parse_uri_file(newdir, &suri, &nsp, &rk, peer);
            if (PMIX_SUCCESS == rc) {
                /* go ahead and try to connect */
                pmix_output_verbose(2, pmix_ptl_base_framework.framework_output,
                                    "pmix:server: attempting to connect to %s", suri);
