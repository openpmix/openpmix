                                     ...
                                     pmix_info_t *info, size_t ninfo)
{
    ...
    if (NULL == suri && NULL != mca_ptl_tcp_component.super.uri) {
                                      ...
                                      pmix_info_t iptr[], size_t niptr)
{
    ...
    *myflag = flag;
    ...
    hdr.nbytes = sdsize + strlen(PMIX_VERSION) + 1 + strlen(sec) + 1 \
                ...
                + strlen(gds) + 1 + sizeof(uint32_t) + len \
                + buf.bytes_used;
    ...
    if (0 == flag) {
        ...
        /* we are a tool that needs an identifier - add our ACLs */
        euid = geteuid();
        u32 = htonl(euid);
        memcpy(msg+csize, &u32, sizeof(uint32_t));
        csize += sizeof(uint32_t);
        ...
        memcpy(msg+csize, &u32, sizeof(uint32_t));
        csize += sizeof(uint32_t);
    } else if (4 == flag || 5 == flag) {
        /* we are a tool that has an identifier - start with our ACLs */
        euid = geteuid();
        u32 = htonl(euid);
        memcpy(msg+csize, &u32, sizeof(uint32_t));
        csize += sizeof(uint32_t);
