...
int pmix_compress_gzip_compress(char * fname, char **cname, char **postfix)
{
    ...
    pmix_compress_gzip_compress_nb(fname, cname, postfix, &child_pid);
...
int pmix_compress_gzip_compress_nb(char * fname, char **cname, char **postfix, pid_t *child_pid)
{
    ...
    *child_pid = fork();
    if( *child_pid == 0 ) { /* Child */
        ...
        if( is_dir ) {
            pmix_asprintf(cname, "%s.tar.gz", base_fname);
            pmix_asprintf(&cmd, "tar -zcf %s %s", *cname, base_fname);
        } else {
            pmix_asprintf(cname, "%s.gz", base_fname);
            pmix_asprintf(&cmd, "gzip %s", base_fname);
        }
        ...
        pmix_output(0, "compress:gzip: compress_nb: Failed to exec child [%s] status = %d\n", cmd, status);
        exit(PMIX_ERROR);
    }
    else if( *child_pid > 0 ) {
        if( is_dir ) {
...
int pmix_compress_gzip_decompress(char * cname, char **fname)
{
    ...
    pmix_compress_gzip_decompress_nb(cname, fname, &child_pid);
    waitpid(child_pid, &status, 0);
...
int pmix_compress_gzip_decompress_nb(char * cname, char **fname, pid_t *child_pid)
{
    ...
    *child_pid = fork();
    if( *child_pid == 0 ) { /* Child */
        ...
        loc_pid = fork();
        if( loc_pid == 0 ) { /* Child */
            pmix_asprintf(&cmd, "gunzip %s", cname);
            ...
            pmix_output(0, "compress:gzip: decompress_nb: Failed to exec child [%s] status = %d\n", cmd, status);
            exit(PMIX_ERROR);
        }
        else if( loc_pid > 0 ) { /* Parent */
            waitpid(loc_pid, &status, 0);
            if( !WIFEXITED(status) ) {
                pmix_output(0, "compress:gzip: decompress_nb: Failed to bunzip the file [%s] status = %d\n", cname, status);
                exit(PMIX_ERROR);
            }
        }
        else {
