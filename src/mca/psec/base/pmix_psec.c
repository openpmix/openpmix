...
#include "src/psec/pmix_native.h"
#if PMIX_WANT_MUNGE
#include "src/psec/pmix_munge.h"
#endif
#
...
static pmix_psec_base_module_t *all[] = {
#if PMIX_WANT_MUNGE
    &pmix_munge_module,
#endif
    &pmix_native_module,
...
pmix_psec_base_module_t pmix_psec = {0};

int pmix_psec_init(void)
{
    pmix_psec_base_module_t *available[PMIX_SEC_NAVAIL];
    int i, j, navail=0;
    char *evar, **options;
    bool exclude;
    ...
    for (i=0; i < navail; i++) {
        if (NULL == available[i]->init) {
            ...
            pmix_psec = *available[i];
            pmix_output_verbose(2, pmix_globals.debug_output,
                                "psec: SPC %s active", pmix_psec.name);
            return PMIX_SUCCESS;
        }
...
void pmix_psec_finalize(void)
{
    /* tell the active module to finalize */
    if (NULL != pmix_psec.finalize) {
        pmix_psec.finalize();
    }
}
