...
static pmix_status_t server_hndshk(struct pmix_peer_t *peer);
...
static pmix_status_t server_hndshk(struct pmix_peer_t *peer)
{
    ...
    if (PMIX_SUCCESS != (rc = pmix_ptl_base_send_blocking(peer->sd, (char*)&size,
                                                          sizeof(size)))) {
        goto exit;
    }
    /* send handshake message */
    if (PMIX_SUCCESS != (rc = pmix_ptl_base_send_blocking(peer->sd, hndshk_msg,
                                                          size))) {
        goto exit;
    }
    /* recv hadshake status from client */
    if (PMIX_SUCCESS != (rc = pmix_ptl_base_recv_blocking(peer->sd, (char*)&status,
                                                          sizeof(status)))) {
        goto exit;
    }
    rc = status;
