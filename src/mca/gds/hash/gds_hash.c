                                         ...
                                         pmix_info_t info[], size_t ninfo)
{
    ...
    for (n=0; n < ninfo; n++) {
        ...
        if (PMIX_CHECK_KEY(&info[n], PMIX_SESSION_ID)) {
            ...
            if (PMIX_SUCCESS != (rc = pmix_gds_hash_process_job_array(&info[n], trk, &flags, &procs, &nodes))) {
                PMIX_ERROR_LOG(rc);
                goto release;
            }
            ...
            if (PMIX_SUCCESS != (rc = pmix_gds_hash_process_node_array(&info[n].value, &trk->nodeinfo))) {
                PMIX_ERROR_LOG(rc);
                goto release;
            }
                                         ...
                                         pmix_buffer_t *buf)
{
    ...
    while (PMIX_SUCCESS == rc) {
        ...
        if (PMIX_CHECK_KEY(kptr, PMIX_PROC_BLOB)) {
            ...
            if (PMIX_SUCCESS != (rc = pmix_gds_hash_process_node_array(kptr->value, &trk->nodeinfo))) {
                PMIX_ERROR_LOG(rc);
                PMIX_RELEASE(kptr);
                return rc;
            }
