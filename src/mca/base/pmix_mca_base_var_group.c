...
PMIX_CLASS_INSTANCE(pmix_mca_base_var_group_t, pmix_object_t, pmix_mca_base_var_group_constructor,
...
int pmix_mca_base_var_group_get_internal(const int group_index, pmix_mca_base_var_group_t **group,
                                         bool invalidok)
{
    if (group_index < 0) {
                      ...
                      const char *component_name, bool invalidok)
{
    ...
    ret = pmix_mca_base_var_generate_full_name4(project_name, framework_name, component_name, NULL,
                                                &full_name);
    if (PMIX_SUCCESS != ret) {
        return PMIX_ERROR;
    }
...
int pmix_mca_base_var_group_find(const char *project_name, const char *framework_name,
...
int pmix_mca_base_var_group_add_var(const int group_index, const int param_index)
{
    ...
    if (PMIX_SUCCESS != (ret = pmix_value_array_append_item(&group->group_vars, &param_index))) {
        return ret;
    }
