...
const char *pmix_var_type_names[] = {"int",
                                     "unsigned_int",
                                     "unsigned_long",
                                     "unsigned_long_long",
                                     "size_t",
...
const size_t pmix_var_type_sizes[] = {sizeof(int),           sizeof(unsigned),
...
static void resolve_relative_paths(char **file_prefix, char *file_path, bool rel_path_search,
                                   char **files, char sep)
{
    char *tmp_str;
    /*
 ...
 */
int pmix_mca_base_var_get_value(int vari, void *value, pmix_mca_base_var_source_t *source,
...
static int int_from_string(const char *src, pmix_mca_base_var_enum_t *enumerator,
                           uint64_t *value_out)
{
    uint64_t value;
    bool is_int;
...
static int var_set_from_string(pmix_mca_base_var_t *var, char *src)
{
    ...
    case PMIX_MCA_BASE_VAR_TYPE_SIZE_T:
        ...
        if (PMIX_ERR_VALUE_OUT_OF_BOUNDS == ret
            || (PMIX_MCA_BASE_VAR_TYPE_INT == var->mbv_type
            ...
            if (var->mbv_enumerator) {
                ...
                pmix_show_help("help-pmix-mca-var.txt", "invalid-value", true, var->mbv_full_name,
                               src);
            }
                    ...
                    const char *component_name, const char *variable_name, bool invalidok)
{
    ...
    ret = pmix_mca_base_var_generate_full_name4(NULL, framework_name, component_name, variable_name,
                                                &full_name);
    if (PMIX_SUCCESS != ret) {
        return PMIX_ERROR;
    }
...
static int fixup_files(char **file_list, char *path, bool rel_path_search, char sep)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (NULL == tmp_file) {
            pmix_show_help("help-pmix-mca-var.txt", "missing-param-file", true, getpid(), files[i],
                           msg_path);
...
static int register_variable(const char *project_name, const char *framework_name,
                             ...
                             pmix_mca_base_var_flag_internal_t flags,
                             int synonym_for, void *storage)
{
    ...
    if (0 > var_index) {
        ...
        pmix_hash_table_set_value_ptr(&pmix_mca_base_var_index_hash, var->mbv_full_name,
        ...
        if (0 != compare_strings(framework_name, group->group_framework)
            ...
            pmix_show_help("help-pmix-mca-var.txt", "var-name-conflict", true, var->mbv_full_name,
...
#if PMIX_ENABLE_DEBUG
            pmix_show_help("help-pmix-mca-var.txt", "re-register-with-different-type", true,
                           var->mbv_full_name);
#endif
            return PMIX_ERR_VALUE_OUT_OF_BOUNDS;
        }
    ...
    if (!original) {
        ...
        if ((PMIX_MCA_BASE_VAR_TYPE_STRING == type || PMIX_MCA_BASE_VAR_TYPE_VERSION_STRING == type)
            && NULL != ((char **) storage)[0]) {
                               ...
                               void *storage)
{
    return register_variable(project_name, framework_name, component_name, variable_name,
                             description, type, PMIX_MCA_BASE_VAR_FLAG_NONE, 0, storage);
}
                                         ...
                                         pmix_mca_base_var_type_t type, void *storage)
{
    return pmix_mca_base_var_register(framework->framework_project, framework->framework_name,
                                      "base", variable_name, help_msg, type, storage);
}
...
int pmix_mca_base_var_register_synonym(int synonym_for, const char *project_name,
                                       ...
                                       const char *synonym_name, pmix_mca_base_var_syn_flag_t flags)
{
    pmix_mca_base_var_flag_internal_t var_flags = PMIX_MCA_BASE_VAR_FLAG_SYNONYM;
    pmix_mca_base_var_t *var;
    int ret;
    ...
    return register_variable(project_name, framework_name, component_name, synonym_name,
                             var->mbv_description, var->mbv_type, var_flags,
                             synonym_for, NULL);
}
...
static int var_set_from_env(pmix_mca_base_var_t *var, pmix_mca_base_var_t *original)
{
    ...
    if (PMIX_MCA_BASE_VAR_SOURCE_OVERRIDE == original->mbv_source) {
        if (!pmix_mca_base_var_suppress_override_warning) {
            pmix_show_help("help-pmix-mca-var.txt", "overridden-param-set", true, var_full_name);
        }
 ...
 */
static int var_set_from_file(pmix_mca_base_var_t *var, pmix_mca_base_var_t *original,
                             pmix_list_t *file_values)
{
    ...
    PMIX_LIST_FOREACH (fv, file_values, pmix_mca_base_var_file_value_t) {
        ...
        if (PMIX_MCA_BASE_VAR_SOURCE_OVERRIDE == original->mbv_source) {
            if (!pmix_mca_base_var_suppress_override_warning) {
                pmix_show_help("help-pmix-mca-var.txt", "overridden-param-set", true,
                               var_full_name);
            }
...
static char *source_name(pmix_mca_base_var_t *var)
{
    ...
    if (PMIX_MCA_BASE_VAR_SOURCE_FILE == var->mbv_source
        || PMIX_MCA_BASE_VAR_SOURCE_OVERRIDE == var->mbv_source) {
...
int pmix_mca_base_var_check_exclusive(const char *project, const char *type_a,
                                      ...
                                      const char *param_b)
{
    ...
    if (PMIX_MCA_BASE_VAR_SOURCE_DEFAULT != var_a->mbv_source
        ...
        /* Print it all out */
        pmix_show_help("help-pmix-mca-var.txt", "mutually-exclusive-vars", true,
...
int pmix_mca_base_var_dump(int vari, char ***out, pmix_mca_base_var_dump_type_t output_type)
{
    ...
    if (PMIX_MCA_BASE_VAR_DUMP_PARSABLE == output_type) {
        ...
        *out = (char **) calloc(line_count + 1, sizeof(char *));
        ...
        ret = asprintf(out[0] + line++, "%sdeprecated:%s", tmp,
