                                       ...
                                       pmix_data_type_t type)
{
    ...
    case PMIX_PROC_INFO:
        rc = asprintf(output,
                      ...
                      (unsigned long) src->data.pinfo->proc.rank, prefx, src->data.pinfo->hostname,
                                       ...
                                       pmix_data_type_t type)
{
    ...
    if (0 < src->nqual) {
        for (n = 0; n < src->nqual; n++) {
            if (PMIX_SUCCESS
                != (rc = pmix20_bfrop_print_info(&t2, p2, &src->qualifiers[n], PMIX_PROC))) {
                free(p2);
                goto done;
            }
            if (0 > asprintf(&t3, "%s\n%s", tmp, t2)) {
...
pmix_status_t pmix20_bfrop_print_alloc_directive(char **output, char *prefix,
                                                 pmix_alloc_directive_t *src, pmix_data_type_t type)
{
    char *prefx;
