                                             ...
                                             void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    ...
    for (i = 0; i < (*num_vals); ++i) {
        ...
        if (0 >= len) { /* zero-length string - unpack the NULL */
            ...
            sdest[i] = (char *) pmix_calloc(len, sizeof(char)); // NULL terminator is included
            if (NULL == sdest[i]) {
                                          ...
                                          pmix_value_t *val)
{
    ...
    switch (val->type) {
        ...
        case PMIX_DATA_ARRAY:
            /* this is now a pointer, so allocate storage for it */
            val->data.darray = (pmix_data_array_t *) pmix_calloc(1, sizeof(pmix_data_array_t));
            ...
            break;
        case PMIX_REGATTR:
            val->data.ptr = (pmix_regattr_t *) pmix_calloc(1, sizeof(pmix_regattr_t));
            ...
            return ret;
        case PMIX_COORD:
            val->data.coord = (pmix_coord_t *) pmix_calloc(1, sizeof(pmix_coord_t));
        ...
        case PMIX_GEOMETRY:
            ...
            return ret;
        case PMIX_DEVICE:
            PMIX_DEVICE_CREATE(val->data.device, 1);
            if (NULL == val->data.device) {
                return PMIX_ERR_NOMEM;
            }
        ...
        case PMIX_DATA_BUFFER:
            ...
            return ret;
        case PMIX_NODE_PID:
            val->data.nodepid = PMIx_Node_pid_create(1);
            if (NULL == val->data.nodepid) {
                return PMIX_ERR_NOMEM;
            }
                                          ...
                                          void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (0 < nbytes) {
            ...
            ptr[i].base_ptr = (char *) pmix_calloc(nbytes, sizeof(char));
                                           ...
                                           void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        /* allocate the space */
        ptr[i].value = (pmix_value_t *) pmix_calloc(1, sizeof(pmix_value_t));
        if (NULL == ptr[i].value) {
            return PMIX_ERR_NOMEM;
        }
        /* unpack the value */
                                         ...
                                         void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (0 < ptr[i].size) {
            ...
            ptr[i].bytes = (char *) pmix_calloc(ptr[i].size, sizeof(char));
            if (NULL == ptr[i].bytes) {
                ptr[i].size = 0;
                return PMIX_ERR_NOMEM;
            }
                                            ...
                                            void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (0 < nkeys) {
            ...
            /* unpack the keys */
            if (NULL == (ptr[i].keys = (char **) pmix_calloc(nkeys + 1, sizeof(char *)))) {
                return PMIX_ERR_NOMEM;
            }
            /* unpack keys */
            m = nkeys;
                                            ...
                                            void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (0 < ptr[i].dims) {
            ...
            ptr[i].coord = (uint32_t *) pmix_calloc(ptr[i].dims, sizeof(uint32_t));
            if (NULL == ptr[i].coord) {
                ptr[i].dims = 0;
                return PMIX_ERR_NOMEM;
            }
                                              ...
                                              void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (0 < nd) {
            ...
            /* unpack the description */
            if (NULL == (ptr[i].description = (char **) pmix_calloc(nd + 1, sizeof(char *)))) {
                return PMIX_ERR_NOMEM;
            }
            m = nd;
                                            ...
                                            void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (0 < ptr[i].len) {
            ...
            ptr[i].bytes = (uint8_t *) pmix_calloc(ptr[i].len, sizeof(uint8_t));
            if (NULL == ptr[i].bytes) {
                ptr[i].len = 0;
                return PMIX_ERR_NOMEM;
            }
                                               ...
                                               pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (0 < ptr[i].ncoords) {
            ...
            /* allocate the coords */
            ptr[i].coordinates = (pmix_coord_t *) pmix_calloc(ptr[i].ncoords, sizeof(pmix_coord_t));
...
pmix_status_t pmix_bfrops_base_unpack_device(pmix_pointer_array_t *regtypes, pmix_buffer_t *buffer,
                                             void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    pmix_device_t *ptr;
    int32_t i, n, m;
    ...
    pmix_output_verbose(20, pmix_bfrops_base_framework.framework_output,
                        "pmix_bfrop_unpack: %d devices", *num_vals);

    PMIX_HIDE_UNUSED_PARAMS(type);

    ptr = (pmix_device_t *) dest;
    n = *num_vals;

    for (i = 0; i < n; ++i) {
        PMIX_DEVICE_CONSTRUCT(&ptr[i]);
        /* unpack the uuid */
        m = 1;
        PMIX_BFROPS_UNPACK_TYPE(ret, buffer, &ptr[i].uuid, &m, PMIX_STRING, regtypes);
        if (PMIX_SUCCESS != ret) {
                                               ...
                                               pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (0 < ptr[i].endpt.size) {
            ...
            ptr[i].endpt.bytes = (char *) pmix_calloc(ptr[i].endpt.size, sizeof(char));
                                           ...
                                           void *dest, int32_t *num_vals, pmix_data_type_t type)
{
    ...
    for (i = 0; i < n; ++i) {
        ...
        if (0 < ptr[i].bytes_used) {
            ...
            ptr[i].base_ptr = pmix_calloc(ptr[i].bytes_used, sizeof(char));
            if (NULL == ptr[i].base_ptr) {
                ptr[i].bytes_used = 0;
                return PMIX_ERR_NOMEM;
            }
...
pmix_status_t pmix_bfrops_base_unpack_nodepid(pmix_pointer_array_t *regtypes,
                                              pmix_buffer_t *buffer, void *dest,
                                              int32_t *num_vals, pmix_data_type_t type)
{
    pmix_node_pid_t *ptr;
    ...
    pmix_output_verbose(20, pmix_bfrops_base_framework.framework_output,
                        "pmix_bfrop_unpack: %d node_pid", *num_vals);

    PMIX_HIDE_UNUSED_PARAMS(type);

    ptr = (pmix_node_pid_t *) dest;
    n = *num_vals;

    for (i = 0; i < n; ++i) {
        PMIx_Node_pid_construct(&ptr[i]);
        /* unpack the hostname */
        m = 1;
        PMIX_BFROPS_UNPACK_TYPE(ret, buffer, &ptr[i].hostname, &m, PMIX_STRING, regtypes);
        if (PMIX_SUCCESS != ret) {
        ...
        /* unpack the nodeid */
        m = 1;
        PMIX_BFROPS_UNPACK_TYPE(ret, buffer, &ptr[i].nodeid, &m, PMIX_UINT32, regtypes);
        if (PMIX_SUCCESS != ret) {
            return ret;
        }
        /* unpack pid */
