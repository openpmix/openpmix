...
int pmix_bfrop_base_select(void)
{
    ...
    PMIX_LIST_FOREACH (cli, &pmix_bfrops_base_framework.framework_components,
        ...
        pmix_output_verbose(5, pmix_bfrops_base_framework.framework_output,
                            "mca:bfrops:select: checking available component %s",
                            component->pmix_mca_component_name);

        /* If there's no query function, skip it */
        if (NULL == component->pmix_mca_query_component) {
            pmix_output_verbose(5, pmix_bfrops_base_framework.framework_output,
                                "mca:bfrops:select: Skipping component [%s]. It does not implement "
                                "a query function",
                                component->pmix_mca_component_name);
            continue;
        }
        ...
        if (PMIX_SUCCESS != rc || NULL == module) {
            pmix_output_verbose(
                5, pmix_bfrops_base_framework.framework_output,
        ...
        PMIX_LIST_FOREACH (mod, &pmix_bfrops_globals.actives, pmix_bfrops_base_active_module_t) {
            if (priority > mod->pri) {
                pmix_list_insert_pos(&pmix_bfrops_globals.actives, (pmix_list_item_t *) mod,
                                     &newmodule->super);
                inserted = true;
                break;
            }
    ...
    if (4 < pmix_output_get_verbosity(pmix_bfrops_base_framework.framework_output)) {
        ...
        PMIX_LIST_FOREACH (mod, &pmix_bfrops_globals.actives, pmix_bfrops_base_active_module_t) {
            pmix_output(0, "\tBfrop: %s Priority: %d", mod->component->base.pmix_mca_component_name,
                        mod->pri);
        }
    }
