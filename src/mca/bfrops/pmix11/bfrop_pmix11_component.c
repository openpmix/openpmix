...
#include "src/mca/bfrops/pmix11/bfrop_pmix11.h"
 ...
 */
pmix_bfrops_base_component_t mca_bfrops_pmix11_component = {
    .base = {
        PMIX_BFROPS_BASE_VERSION_1_0_0,

        /* Component name and version */
        .pmix_mca_component_name = "pmix11",
...
pmix_status_t component_open(void)
{
    ...
    PMIX_CONSTRUCT(&mca_bfrops_pmix11_component.types, pmix_pointer_array_t);
...
pmix_status_t component_query(pmix_mca_base_module_t **module, int *priority)
{
    ...
    *priority = mca_bfrops_pmix11_component.priority;
    *module = (pmix_mca_base_module_t *)&pmix_bfrops_pmix11_module;
    return PMIX_SUCCESS;
}
...
{
    PMIX_DESTRUCT(&mca_bfrops_pmix11_component.types);
    return PMIX_SUCCESS;
}

static pmix_bfrops_module_t* assign_module(const char *version)
{
    if (NULL != version && NULL == strstr(version, "pmix11")) {
        return NULL;
    }
    return &pmix_bfrops_pmix11_module;
}
                                       ...
                                       char *tmpdir)
{
    ...
    pid = getpid();
    if (0 > asprintf(&pmix_pid, "%s/pmix-%d", tmpdir, pid)) {
