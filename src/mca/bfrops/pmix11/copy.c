...
pmix_status_t pmix11_copy(void **dest, void *src,
                          pmix_data_type_t type)
{
    /* kick the process off by passing this in to the base */
    return pmix_bfrops_base_copy(&mca_bfrops_pmix11_component.types,
                                 dest, src, type);
}

pmix_status_t pmix11_std_copy(void **dest, void *src,
                              pmix_data_type_t type)
{
    size_t datasize;
    uint8_t *val = NULL;
