...
establish the type.** They are the only place in this file that reaches
into a `pmix_value_t` union, and they did it on the key alone: a
`PMIX_GROUP_MEMBERSHIP` carrying anything but a proc array dereferenced
whatever shared the union — including the NULL `darray` that is a legal
value for a `pmix_value_t` tagged `PMIX_DATA_ARRAY` — and a mistyped
`PMIX_GROUP_ID` or `PMIX_EVENT_AFFECTED_PROC` reached `strcmp()` and
`PMIX_CHECK_PROCID` as garbage pointers. All three are screened now, the
way `src/client/pmix_client_group.c` screens the same fields on the
handlers' side. This is that directory's signature defect (see its
fourth and fifth sweeps) and it applies here for exactly the same
reason: the event was generated by another process.
