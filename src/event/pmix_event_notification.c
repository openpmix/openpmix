...
void pmix_invoke_local_event_hdlr(pmix_event_chain_t *chain)
{
    ...
    if (PMIX_GROUP_CONSTRUCT_COMPLETE == chain->status) {
        /* Every field here is another process's word for what it holds: the
         * event was generated by the group's leader (or by a server on its
         * behalf) and unpacked off the wire, so the key does not establish
         * the type. A PMIX_GROUP_MEMBERSHIP carrying anything but a proc
         * array made the two reads below dereference whatever shared the
         * union - including a NULL darray, which is a legal value for a
         * pmix_value_t tagged PMIX_DATA_ARRAY - and a mistyped
         * PMIX_GROUP_ID reached strcmp() as a garbage pointer. */
        // find the group ID and membership
        for (n = 0; n < chain->ninfo; n++) {
            if (PMIX_CHECK_KEY(&chain->info[n], PMIX_GROUP_MEMBERSHIP)) {
                if (PMIX_DATA_ARRAY == chain->info[n].value.type &&
                    NULL != chain->info[n].value.data.darray &&
                    PMIX_PROC == chain->info[n].value.data.darray->type &&
                    NULL != chain->info[n].value.data.darray->array) {
                ...
                if (PMIX_STRING == chain->info[n].value.type &&
                    NULL != chain->info[n].value.data.string) {
        ...
        if (NULL != members && 0 < nmembers && NULL != grpid) {
            ...
            if (NULL == grp) {
                ...
                if (NULL != grp) {
                    ...
                    if (NULL == grp->members) {
                        /* better to know nothing of the group than to record
                         * it with an empty membership every later operation
                         * would then read */
                        PMIX_RELEASE(grp);
                    } else {
    ...
    if (PMIX_GROUP_LEFT == chain->status) {
        /* wire-supplied, as above: PMIX_CHECK_PROCID dereferences what the
         * affected proc points at, and strcmp what the group id does */
        ...
        for (n = 0; n < chain->ninfo; n++) {
            if (PMIX_CHECK_KEY(&chain->info[n], PMIX_GROUP_ID)) {
                ...
                if (PMIX_PROC == chain->info[n].value.type &&
                    NULL != chain->info[n].value.data.proc) {
...
bool pmix_notify_check_range(pmix_range_trkr_t *rng, const pmix_proc_t *proc)
{
    ...
    if (PMIX_RANGE_PROC_LOCAL == rng->range) {
        /* only accept events that were generated by our own process */
        return PMIX_CHECK_PROCID(proc, &pmix_globals.myid);
    }
