...
## Build the Release
- [x] Verify the major,minor,release,greek version numbers in `VERSION`
- [x] Update the `c:r:a` shared library version number(s) in `VERSION` [per the GNU Libtool shared library version number rules](https://www.gnu.org/software/libtool/manual/html_node/Updating-version-info.html)
   ...
   - [x] *IF RELEVANT:* If this is a new backwards-compatible release on a single branch (i.e., this is `vx.y.z` where `z>1`), you probably want to examine `git log --stat --no-merges last_release_tag..this_branch_name` to see what source code files have changed (which directly impacts how to increment the `c:r:a` values).
...
- [x] Update all documentation files (especially including dates and version numbers), including:
  - [x] `README`: all relevant updates, build options, etc.  Be sure to update the date near the top of the file.
  - [x] `NEWS`: List all user-noticeable changes.  Similar to setting shared library versions (above): 
  ...
  - [x] `LICENSE`: Update the years in the copyright notices 
 
## Publish pre-releases
- [x] Create a tag for the pre-release, matching the version being released (ie, `git tag -a v3.0.1rc1 -m "v3.0.1rc1" <HASH>`).  Verify <HASH> points to the correct release point.  Push the tag for the release.
- [ ] Create a PR (e.g., tickle README.md) to trigger CI and verify cross-version compatibility
- [x] PR release candidate to OMPI for MTT and verify passage (must run for at least one week for coverage)
- [x] Build the pre-release tarball and post it on the GitHub tag
- [x] Send an email to the `pmix@googlegroups.com` and `ompi-packagers@lists.open-mpi.org` mailing lists announcing the pre-release candidate being available for evaluation. Body of message should include the link to the GitHub tag and a copy of the `NEWS` items relating to this candidate.
...
- [ ] Send an email to the `pmix@googlegroups.com` and `ompi-packagers@lists.open-mpi.org`mailing lists announcing the release
