...
AC_DEFUN([PMIX_CASE_SENSITIVE_FS_SETUP],[
#
# Arguments: none
#
# Dependencies: None
...
# (like HFS+ on MacOS X and MS Windows) are not case sensitive - pmixcc
# and pmixCC are the same file.
#
...
if test "$with_cs_fs" = "yes"; then
    ...
    PMIX_WANT_CS_FS=$have_cs_fs
else
    PMIX_WANT_CS_FS=0
fi

if test "$PMIX_WANT_CS_FS" = "1"; then
    AC_MSG_RESULT([yes])
else
    AC_MSG_RESULT([no])
fi
