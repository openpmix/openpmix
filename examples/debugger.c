...
static pmix_status_t spawn_debugger(char *appspace)
{
    ...
    PMIX_INFO_LOAD(&dinfo[0], PMIX_MAPBY, "ppr:1:node",
    ...
    PMIX_INFO_LOAD(&dinfo[3], PMIX_NOTIFY_COMPLETION, NULL,
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (0 == strcmp(argv[i], "-a") || 0 == strcmp(argv[i], "--attach")) {
            if (NULL != nspace) {
     ...
     * job completes */
    DEBUG_CONSTRUCT_LOCK(&mylock);
    PMIx_Register_event_handler(&code, 1, NULL, 0, release_fn, evhandler_reg_callbk,
                                (void *) &mylock);
    DEBUG_WAIT_THREAD(&mylock);
    DEBUG_DESTRUCT_LOCK(&mylock);
    ...
    if (NULL != nspace) {
        ...
        if (PMIX_SUCCESS
            != (rc = PMIx_Query_info_nb(query, nq, querycbfunc, (void *) &myquery_data))) {
        ...
        for (n = 0; n < myquery_data.ninfo; n++) {
            if (0 == strcmp(myquery_data.info[n].key, PMIX_QUERY_SPAWN_SUPPORT)) {
                ...
                if (NULL
                    != strstr(myquery_data.info[n].value.data.string, PMIX_DEBUG_STOP_ON_EXEC)) {
                    stop_on_exec = true;
                } else {
                    stop_on_exec = false;
                }
        ...
        if (cospawn) {
            ...
            if (stop_on_exec) {
                PMIX_INFO_LOAD(&info[1], PMIX_DEBUG_STOP_ON_EXEC, NULL,
                               PMIX_BOOL); // procs are to stop on first instruction
            } else {
                PMIX_INFO_LOAD(&info[1], PMIX_DEBUG_STOP_IN_INIT, NULL,
