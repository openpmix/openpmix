...
# $HEADER$
#
# Python port of examples/group_invite_timeout.c
#
# One invitee never answers the invitation. With PMIX_GROUP_OPTIONAL set
# on the invite, the library must:
#   - fire PMIX_GROUP_INVITE_FAILED at the leader naming the
#     non-responder;
...
def invite_handler(evhdlr, status, source, info, results):
    ...
    if myproc['rank'] == victim:
        # the non-responder: deliberately do NOT answer so the leader's
        # timeout fires
        eprint("%s:%d INVITED but deliberately NOT responding "
               "(drives timeout)" % (myproc['nspace'], myproc['rank']))
        return PMIX_EVENT_ACTION_COMPLETE, None

    grp = find_key(info, PMIX_GROUP_ID)
    eprint("%s:%d INVITED - accepting" % (myproc['nspace'], myproc['rank']))
...
# PMIX_GROUP_INVITE_FAILED handler (leader) - records who failed to
# respond
...
def main():
    ...
    victim = nprocs - 1
    eprint("Client %s:%d job size %d (non-responder is rank %d)"
    ...
    if 0 == myproc['rank']:
        eprint("%d executing Group_invite (timeout %d s) for the whole job"
               % (myproc['rank'], INVITE_TIMEOUT))
        procs = [{'nspace': myproc['nspace'], 'rank': n}
                 for n in range(nprocs)]
        # mark participation PMIX_GROUP_OPTIONAL so a non-responder yields
        # a reduced group rather than aborting the construct
        tinfo = [{'key': PMIX_TIMEOUT, 'value': INVITE_TIMEOUT,
                  'val_type': PMIX_INT},
                 {'key': PMIX_GROUP_OPTIONAL, 'value': True,
                  'val_type': PMIX_BOOL}]
        ...
        if failed_proc.get('rank') != victim:
            eprint("Client ns %s rank %d: FAILED - wrong failed rank %d "
                   "(expected %d)"
                   % (myproc['nspace'], myproc['rank'],
                      failed_proc.get('rank', PMIX_RANK_UNDEF), victim))
            return done(PMIX_ERROR)
        eprint("%d PMIX_GROUP_INVITE_FAILED for non-responder: PASS"
               % myproc['rank'])
    elif myproc['rank'] == victim:
        # the non-responder does not join; it only reports and waits at
        # the closing barrier
        eprint("%d not joining the group (non-responder)" % myproc['rank'])

    # the leader and the accepting members must be notified of completion;
    # the non-responder must NOT be (it is not part of the group)
    if myproc['rank'] != victim:
    ...
    # final job-level sync so every rank (including the non-responder)
    # tears down together
    rc = client.fence([proc], None)
    if PMIX_SUCCESS != rc:
