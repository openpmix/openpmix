...
# $HEADER$
#
# Python port of examples/group_destruct_die.c
#
# The destruct counterpart of group_die: everyone constructs the group,
# then the last rank leaves before contributing to the DEstruct (after a
# short delay, so the others have entered it and their servers have
# created the local trackers) and WITHOUT calling finalize, so the server
# sees a dropped connection. The server must account for the departed
# member and complete the destruct on the survivors rather than hang - the
# same lost-connection accounting that completes a construct
# (account_departed() in src/server/pmix_server_group.c) applies to a
# destruct block, forwarding blk->grpop == PMIX_GROUP_DESTRUCT to the
# host. See docs/how-things-work/collectives.
#
...
def main():
    ...
    # every rank (including the victim) constructs the group so a live
    # group exists on every server before anyone attempts to destruct it
    eprint("%d executing Group_construct" % myproc['rank'])
    procs = [{'nspace': myproc['nspace'], 'rank': n} for n in range(nprocs)]
    rc, results = client.group_construct("ddgroup", procs, None)
    if PMIX_SUCCESS != rc:
        eprint("Client ns %s rank %d: PMIx_Group_construct failed: %s"
               % (myproc['nspace'], myproc['rank'], client.error_string(rc)))
        sys.exit(1)

    # sync so the group is fully constructed everywhere before anyone
    # leaves
    rc = client.fence([proc], None)
    if PMIX_SUCCESS != rc:
    ...
    if myproc['rank'] == victim:
        ...
        # destruct and their servers have built the local trackers, then
        # leave without calling it - and without calling finalize, so the
        # server sees a dropped connection and must run its
        # lost-connection accounting. We exit with status 0 so the
        # launcher does not abort the whole job; the dropped socket, not
        # the exit code, is what exercises the server
        eprint("%d leaving BEFORE group destruct (simulated failure)"
               % myproc['rank'])
        time.sleep(2)
        os._exit(0)

    eprint("%d executing Group_destruct" % myproc['rank'])
    rc = client.group_destruct("ddgroup", None)

    # The destruct must COMPLETE rather than hang. It may return success
    ...
    # participant
    eprint("%d Group destruct complete on survivors: status %s"
